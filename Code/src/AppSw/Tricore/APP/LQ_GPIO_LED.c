...
void GPIO_LED_Init(void)
{
	  ...
	  PIN_InitConfig(LED4p, PIN_MODE_OUTPUT, 0);
}
...
void LED_Ctrl(LEDn_e LEDno, LEDs_e sta)
{
    ...
    break;
    case LED4:
          if(sta==ON)        PIN_Write(LED4p,0);
          else if(sta==OFF) PIN_Write(LED4p,1);
          else if(sta==RVS) PIN_Reverse(LED4p);
        break;
    case LEDALL:
      if(sta==ON)
      {
