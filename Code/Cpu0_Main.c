...
int core0_main (void)
{
    ...
    STM_InitConfig(STM1, STM_Channel_1, 100);//每一秒发送一次数据，由于串口速率不够，所有实际情况可能每30s接收一次
    STM_InitConfig(STM1, STM_Channel_0, 20000);//每20ms进入一次，频率50hz,编码器读取与pid计算
...
//        OLED_P8x16Str(0, 0,txt);       //显示字符串
//        if(KEY_Read(KEY1)==0)//下
//      {
//        //  CPSR_Trig(IfxSrc_Tos_cpu1,SoftIRQ1);//双核互斥
//         //   currentDesired1++;
//            speedSet++;
//          LED_Ctrl(LEDALL,ON);
//          delayms(100);
//      }
//        else if(KEY_Read(KEY0)==0)//上
//        {  LED_Ctrl(LEDALL,OFF);
//        speedSet--;
//      //  currentDesired1--;
//        delayms(100);
